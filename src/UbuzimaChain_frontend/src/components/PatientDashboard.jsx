...
const PatientDashboard = ({ userId, token, setError }) => {
  ...
  // Fetch patient details when actor or userId changes
  useEffect(() => {
    const fetchPatientDetails = async () => {
      ...
      try {
        ...
        if (result.Ok) {
          ...
          // Throw an error if result.Err is present
          throw new Error(result.Err || 'Failed to fetch patient details');
        }
      } catch (err) {
        console.error('Error fetching patient details:', err);
        // Suppress error if patient isn't registered yet
  ...
  return (
    <div className="space-y-6">
      ...
      <ErrorBoundary>
        {!patientDetails ? (
          // If no patient details are found, show the registration form
          <div className="bg-white shadow rounded-lg p-6">
            <PatientRegistration
              userId={userId}
              token={token}
