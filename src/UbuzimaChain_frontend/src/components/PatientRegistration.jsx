...
const PatientRegistration = ({ userId, token }) => {
    ...
    const createActor = async () => {
        ...
        const host = process.env.DFX_NETWORK === "ic" ? "https://boundary.ic0.app" : "http://localhost:4943";
    ...
    const validateForm = () => {
        ...
        for (const contact of emergency_contacts) {
            ...
            if (!phoneRegex.test(contact.phone)) {
                setMessage({
                    type: "error",
                    content: "Please enter a valid phone number for emergency contacts (e.g., +250781932205 or 250781932205).",
                });
                return false;
            }
        }
    ...
    const handleSubmit = async (e) => {
        ...
        console.log("Current userId:", userId); // Add this to debug
        setLoading(true);
        ...
        try {
            ...
            const registrationPayload = {
                user_id: userId,  // Remove toString() if present
                full_name: formData.full_name,
                date_of_birth: formData.date_of_birth,
                gender: formData.gender,
              blood_type: formData.blood_type ? [formData.blood_type] : [],  // Changed from array
            ...
            console.log("Submitting payload:", registrationPayload);

    
    const result = await actor.register_patient(registrationPayload);
